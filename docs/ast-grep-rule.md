- Fetch https://ast-grep.github.io/llms.txt to read full documentation on `ast-grep`
...
Pattern must be quoted by single quote ' to prevent shell from interpreting $ sign. ast-grep -p '$PROP && $PROP()' is okay.
